    ...
    public class UnitAttack : FactionEntityAttack
    {
        ...
        public override ErrorMessage SetTargetLocal (SetTargetInputData input)
        {
            ...
            if (IsTargetInRange(transform.position, input.target)
                && !LineOfSight.IsObstacleBlocked(transform.position, RTSHelper.GetAttackTargetPosition(input.target))) 
            ...
            else if (LineOfSight.IsObstacleBlocked(input.target.position, RTSHelper.GetAttackTargetPosition(input.target)))
                errorMessage = ErrorMessage.LOSObstacleBlocked;
